...
# 复制generate_chapter_html.py中的函数和配置
class Config:
    # 页面尺寸（用于打印样式）
    PAGE_WIDTH_MM = 148
    PAGE_HEIGHT_MM = 210
...
def generate_html(markdown_file, output_file, config):
    ...
    print("预处理Markdown...")
    processed_markdown = preprocess_markdown(markdown_content, config)
...
def main():
    ...
    # 创建报告
    report_content = f"""# 儿童版中国哲学史 - 批量HTML样张生成报告

## 基本信息
- 处理时间: 2026-04-02 21:17
- 目标章节: 第2章至第12章 (共11章)
...
"""
    ...
    # 保存报告
    report_file = Path("outputs/儿童哲学史/排版阶段/批量生成报告.md")
