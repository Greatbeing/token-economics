...
description: >
  当用户要求在飞书云文档中绘制图表，或使用飞书画板绘制架构图、流程图、思维导图、时序图或其他可视化图表时使用此 skill。实际任务执行时只在主agent执行，不要在子agent执行。
...
> 上传需要飞书认证。遇到认证或权限错误时，阅读 [`../lark-shared/SUB_SKILL.md`](../lark-shared/SUB_SKILL.md) 了解登录和权限处理。
...
关于飞书文档的创建，读取等更多操作，请参考 lark-doc skill [`../lark-doc/SUB_SKILL.md`](../lark-doc/SUB_SKILL.md)。
