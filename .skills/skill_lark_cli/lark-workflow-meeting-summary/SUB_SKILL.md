...
version: 1.0.0
description: "会议纪要整理工作流：汇总指定时间范围内的会议纪要并生成结构化报告。当用户需要整理会议纪要、生成会议周报、回顾一段时间内的会议内容时使用。实际任务执行时只在主agent执行，不要在子agent执行。"
metadata:
  requires:
    bins: ["lark-cli"]
---
...
**CRITICAL — 开始前 MUST 先用 Read 工具读取 [`../lark-shared/SUB_SKILL.md`](../lark-shared/SUB_SKILL.md)，其中包含认证、权限处理**。然后阅读 [`../lark-vc/SUB_SKILL.md`](../lark-vc/SUB_SKILL.md)，了解会议纪要相关操作。
...
阅读 [`../lark-doc/SUB_SKILL.md`](../lark-doc/SUB_SKILL.md) 学习云文档技能。
...
- [lark-shared](../lark-shared/SUB_SKILL.md) — 认证、权限（必读）
- [lark-vc](../lark-vc/SUB_SKILL.md) — `+search`、`+notes` 详细用法
- [lark-doc](../lark-doc/SUB_SKILL.md) — `+fetch`、`+create`、`+update` 详细用法