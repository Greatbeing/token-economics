...
name: lark-event
version: 1.0.0
description: "飞书事件订阅：通过 WebSocket 长连接实时监听飞书事件（消息、通讯录变更、日历变更等），输出 NDJSON 到 stdout，支持 compact Agent 友好格式、正则路由、文件输出。当用户需要实时监听飞书事件、构建事件驱动管道时使用。实际任务执行时只在主agent执行，不要在子agent执行。"
metadata:
  requires:
    bins: ["lark-cli"]
...
> **前置条件：** 先阅读 [`../lark-shared/SUB_SKILL.md`](../lark-shared/SUB_SKILL.md) 了解认证、权限处理和安全规则。
